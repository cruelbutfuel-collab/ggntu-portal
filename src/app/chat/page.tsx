...
function ChatInner() {
  ...
  const INITIAL_MESSAGE: Message = {
    ...
    text: calcMode
      ...
      : hollandMode
        ? hollandSpec
          ? hollandLevel === 'spo'
            ? `Привет! Я <b>Алия</b>. Вижу, тебя интересует <b>${hollandSpec}</b> — это программа колледжа (СПО). Поступление без ЕГЭ, по среднему баллу аттестата. Расскажу подробнее.`
  ...
  useEffect(() => {
    if (hollandMode && !hollandTriggered.current) {
      ...
      const spoNote = hollandLevel === 'spo' ? ' Это программа колледжа (СПО) — поступление без ЕГЭ, по среднему баллу аттестата. Не упоминай ЕГЭ.' : ''
      const apiText = hollandSpec
        ? `Меня заинтересовала специальность "${hollandSpec}".${spoNote} Мой тип по Holland Codes — ${hollandKey}, label: ${hollandLabel || hollandKey}${zoneStr}. Расскажи подробнее.`
